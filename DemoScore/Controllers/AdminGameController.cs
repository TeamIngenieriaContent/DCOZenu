...
namespace DemoScore.Controllers
{
    public class AdminGameController : Controller
    {
        ...
        public IEnumerable<SelectListItem> GetContext( int com)
        {
            List<MG_Context> contexts = ApplicationDbContext.MG_Contexts.Where(x => x.Contex_Id == com).ToList();
            IEnumerable<SelectListItem> Template = contexts.Select(x =>
            new SelectListItem
            {
                Value = x.Contex_Id.ToString(),
                Text = x.Contex_Desc
            });
            return new SelectList(Template, "Value", "Text");
        }
        ...
        public ActionResult MgQuestions(int id)
        {
            ...
            var context = ApplicationDbContext.MG_Contexts.Where(x => x.Company_Id == setting.Company_Id).ToList();
            ...
            if (ModelState.IsValid)
            {
                if (model.cate_Id != 0)
                {
                    ...
                    if (model.sub_Id != 0)
                    {
                        ...
                        var context = ApplicationDbContext.MG_Contexts.Find(model.contex_Id);
